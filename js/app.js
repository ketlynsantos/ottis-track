...
function statusBadge(stage) {
  ...
  if (s.includes("fabr") || s.includes("inst") || s.includes("test"))
    return "inprogress";
  return "inprogress";
}
...
function renderKPIs(el, projs) {
  ...
  const avgLead = Math.round(
    projs.reduce((a, p) => a + (leadTime(p) || 0), 0) / projs.length
  );
  const avgCV = projs.reduce((a, p) => a + costVariance(p), 0) / projs.length;
  const avgFPY = projs.reduce((a, p) => a + p.fpy, 0) / projs.length;
  el.innerHTML = `
    <div class="card kpi"><h3>Marcos no prazo</h3><div class="value">${fmtPct(
      onTime
    )}</div><div class="delta">média geral</div></div>
    <div class="card kpi"><h3>Lead Time Médio</h3><div class="value">${avgLead} dias</div><div class="delta">pedido → entrega</div></div>
    <div class="card kpi"><h3>Variação de custo</h3><div class="value">${(
      avgCV * 100
    ).toFixed(1)}%</div><div class="delta">orçado × realizado</div></div>
    <div class="card kpi"><h3>Primeiro passe (FPY)</h3><div class="value">${fmtPct(
      avgFPY
    )}</div><div class="delta">sem retrabalho</div></div>`;
}
...
function renderProject(p) {
  if (!p) {
    q("#project").innerHTML = "<div class='card'>Projeto não encontrado.</div>";
    return;
  }
  q("#p-title").textContent = `${p.client} — ${p.city}/${p.country} (${p.id})`;
  q("#p-stage").textContent = p.stage;
  q("#p-stage").className = "badge " + statusBadge(p.stage);
  ...
  tl.innerHTML = p.milestones
    .map((m) => {
      ...
      return `<div class="${cls}"><strong>${
        m.name
      }</strong><div class="small">Planejado: ${m.planned || "—"} • Real: ${
        m.actual || "—"
      }</div></div>`;
    })
    .join("");
  q(
    "#cost-line"
  ).textContent = `Orçado: ${p.budget} • Realizado: ${p.actualCost}`;
  const delta = Math.round(((p.actualCost - p.budget) / p.budget) * 1000) / 10;
  q("#cost-delta").textContent = `Δ ${delta}%`;
  const fill = Math.min(
    100,
    Math.max(2, Math.round((p.actualCost / p.budget) * 100))
  );
  q("#cost-bar .fill").style.width = fill + "%";
  const ql = q("#quality");
  ql.innerHTML = p.quality.length
    ? p.quality
        .map(
          (qc) =>
            `<div class="card"><div><strong>${qc.id}</strong> • Gravidade: ${qc.severity} • Status: ${qc.status}</div><div class="small">${qc.title} • Prazo: ${qc.due}</div></div>`
        )
        .join("")
...
function tabTo(n) {
  qa(".tab").forEach((e) => e.classList.remove("active"));
  qa(".tabpanel").forEach((e) => (e.style.display = "none"));
  q(`[data-tab='${n}']`).classList.add("active");
  q(`#panel-${n}`).style.display = "block";
}
...
function renderPortal(p) {
  ...
  q("#steps").innerHTML = STEPS.map(
    (s, i) =>
      `<div class="step ${
        i <= idx ? "done" : ""
      }"><div class="dot"></div><div>${s}</div></div>`
  ).join("");
...
function renderFeedbackDashboard() {
  ...
  function apply() {
    ...
    if (p !== "all") {
      ...
      rows = rows.filter(
        (r) => (now - new Date(r.date)) / (1000 * 60 * 60 * 24) <= days
      );
    }
    // KPIs
    ...
    q("#comments").innerHTML =
      list || '<div class="card">Sem feedbacks no período.</div>';
  }
